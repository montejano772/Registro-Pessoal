...
create table if not exists public.partidas (
  ...
  tipo_tempo text not null default 'individual' check (tipo_tempo in ('individual', 'compartilhado')),
  tempo_compartilhado_restante integer,
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'partidas_host_jogador_id_fkey') then
  ...
  if not exists (select 1 from pg_constraint where conname = 'partidas_jogador_atual_id_fkey') then
    alter table public.partidas
  ...
  if not exists (select 1 from pg_constraint where conname = 'partidas_vencedor_jogador_id_fkey') then
    alter table public.partidas
